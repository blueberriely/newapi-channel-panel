...
拿到状态数据之后，怎么知道某一条状态属于哪个模型：分两层精确匹配，不做子串模糊匹配。第一层先找监控项名字跟模型 id（带路由前缀的完整名字）完全一致的——同一个底层模型不同路由实测可用率可能天差地别（实测过同一个模型两个路由一个 97%、一个 0%），全名对上了就直接用，不再降级。第一层没找到，才退一步找监控项名字跟"去掉路由前缀后的裸模型名"完全一致的，给那些监控只按底层模型整体挂、不分路由的站兜底。两层都忽略大小写、空格、破折号、竖线、括号这些符号差异。

早期版本这里有个模糊匹配兜底——全名对不上时，退回去拿模型的路由前缀、价格分组名这些词跟监控名做子串匹配。实测下来这个兜底比它解决的问题更大：不少中转站的监控页根本不是按单个模型挂的，而是按"分组"或"路由"整体挂一条监控（比如监控项直接叫`cc2分组`，或者一条`[Kiro] Opus`监控背后其实是 4-5 个不同版本的 opus）。这种时候路由/分组词几乎什么都能模糊匹配上，实测出现过把 opus 的状态显示成 sonnet 的、甚至把 Claude 模型的状态显示成 Gemini 模型的。所以现在的逻辑是：监控项名字必须跟模型 id 完全一致才绑定，对不上就让这个模型没有 `status` 字段——`status_summary` 里仍然会带着抓到的原始监控列表，只是不会尝试把它跟某个具体模型强行绑在一起，宁可摆着让你自己看，也不瞎绑。
